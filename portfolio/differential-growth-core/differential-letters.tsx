...
const sketch = function (p5: P5CanvasInstance<MySketchProps>) {
  ...
  p5.setup = function () {
    canvas = p5.createCanvas(900, 400, p5.P2D);
  ...
  function restartWorld() {
    ...
    world.clearPaths();
    const word: string[] = ['E', 'R', 'A', 'Z']

    for (let i = 0; i < word.length; i++) {
        const l: string = word.at(i);
        let letter = createLetter(l);
        let nodes = new Array<Node>;
        const p1: PositionType = { x: border + i * 135 + 25,  y: 45 };
        const p2: PositionType = { x: border + i * 135 + 55, y: 45 };
        nodes.push(new Node(getID(), p5, p1, settings, true));
        nodes.push(new Node(getID(), p5, p2, settings, true));
        for (let j = 0; j < letter.length; j++) {
            letter[j][0] = letter[j][0]  + (140 * i);
        };
